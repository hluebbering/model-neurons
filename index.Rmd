...
x <- seq(-5, 5, 0.01)
plot(x, sigmoid(x, a=1, b=1), col='#0084A1', ylab="Sigmoid Function", xlab="X")
grid()
...
y2 <- Si(x)
## Not run: 
plot(x, y1, type = "l", col = "#A3C893", xlab = "", ylab = "", xlim=c(-2,5), lwd=3)
lines(x, y2, col = "#37B8CB", lwd=3)
grid()
legend(-1,0.8, c(TeX("$S_{excitatory}$"),TeX("$S_{inhibitory}$")), lty = 1,lwd=4, col = c("#A3C893", "#37B8CB"), box.lwd = 0)
```
...
knitr::kable(functions ,col.names = c("", ""), caption = table_cap1) %>%
...
knitr::kables(list(kbl(parameters,col.names = c(" ", " "), booktabs = T,linesep = "", escape = F) %>%
  kable_styling(full_width = FALSE, font_size = 10, wraptable_width = 1, 
                html_font = "Frank-Ruhl-Libre", bootstrap_options = c("hover")) %>% 
  row_spec(0, color = "hotpink") %>%
    column_spec(1, color = "white", background = spec_color(mtcars$mpg[1:nrow(parameters)], begin = 0.1 ,end = 0.7), popover = paste("parameters:", parameters$parameters[1:nrow(parameters)])) %>%
  ...
  kbl(functions ,col.names = c(" ", " "), booktabs = T, linesep = "") %>%
    ...
    column_spec(1, color = "white", background = spec_color(mtcars$mpg[5:nrow(functions)], begin = .7 ,end = .9), 
                popover = paste("functions:", 
                                functions$functions[1:nrow(functions)]), bold = T) %>% 
    add_header_above(c("System Functions"=2),align = "l", line = F)), 
  caption = table_cap2) %>%
  kable_styling(c("striped", "condensed"), full_width = F, position = "c", 
                repeat_header_continued = T, latex_options = "striped") %>%
