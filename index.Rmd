...
neur_cap <- fig_nums(name = "phase1", caption = "Projecting from each cell’s body, called its soma, is a tree of dendrites and a long axon. Dendrites receive the signals sent down the axon.")
```


```{r, fig.cap = neur_cap, out.width=400}
...
```{r include=FALSE}
neur_cap2 <- fig_nums(name = "phase2", caption = "A Single Neuron. Dendrites are branched extensions that can produce electrochemical simulations. The synapses distributed throughout the tree transmit electrical signals to the dendrites.")
```
...
```{r}
neur_cap6 <- fig_nums(name = "phase6", caption = "The myelin space on the axon has high-concentration ion channels, allowing nodes to regenerate the action potential. Positive ion current is directed from inside to outside.")
```
...
#### <span class="sp2">1.</span> Proportion of Sensitive Cells in a Sub-population
...
```{r}
neur_cap5 <- fig_nums(name = "phase5", caption = "Sigmoid Subpopulation Response Function. The particular function shown here is the logistic curve: $S(x)=\\frac{1}{1+e^{-a(x-\\theta)}},$ such that $\\theta=5, a=1$.")
```



```{r, fig.cap=neur_cap5, out.width=500}
...
```{r}
neur_cap6 <- fig_nums(name = "phase6", caption = "Plot of the Sigmoid functions $S_e$ and $S_i$ for the excitatory and inhibitory subpopulations, respectively. Parameters: $a_e = 1.3, \\theta_e=4, a_i=2, \\text{ and } \\theta_i=3.7$")
```


```{r, fig.cap=neur_cap6, out.width=500}
y1 <- Se(x)
y2 <- Si(x)
## Not run: 
...
#### <span class="sp3">2.</span> Subpopulation Response Functions
...
table_cap1 <- tables("1", caption = "System Functions")
```
...
knitr::kable(functions, col.names = c("", "")) %>%
  kable_styling(full_width = FALSE, font_size = 10, html_font = "Frank-Ruhl-Libre", 
                bootstrap_options=c("striped", "hover")) %>% 
  row_spec(0, color = "darkslategray", font_size = 1) %>%
  column_spec(1, color = "white", 
              background = c(colpal2[2], colpal2[3], colpal2[4], colpal2[5]), 
              popover = paste("function:", functions$functions), bold = T) %>%
  kable_paper() %>%
  add_header_above(header = c("Table 1.1: Defining the System Functions" = 2), align = "l", line = F, font_size = 11)
...
#### <span class="sp4">3.</span> Average Level of Excitation 
...
knitr::kable(parameters,col.names = c("", ""), escape = F) %>%
  ...
  column_spec(1, color = "white", 
              background = c(colpal2[2], colpal2[3], colpal2[4], colpal2[5], 
                             colpal2[6], colpal2[7])) %>%
  add_header_above(c("Table 1.2: Defining the System Parameters" = 2), 
                   align = "l", line = T)  %>%
  kable_paper()
```
...
#### <span class="sp5">4.</span> Equations for the activities E(t) and I(t)
...
#### <span class="sp6">5.</span> Dynamics of a Localized Population of Neurons
...
& \cdot \, \mathcal{S}_e
\left\{ \int^t_{-\infty} \alpha(t-t^\prime) [c_1 E(t^\prime) - c_2 I(t^\prime) + P(t^\prime)] dt^\prime \right\}, 
\tag{10} 
\end{align*}
$$
...
\small
\begin{align}
\mathbf{I}(t+\tau^\mathrm{\prime}) \equiv& {\left[ 1-\int^t_{t-r^\mathrm{\prime}}I(t^\prime) \; dt^\prime \right ]} \\
& \cdot \, \mathcal{S}_i
\left\{ \int^t_{-\infty} \alpha(t-t^\prime) [c_3 E(t^\prime) - c_4 I(t^\prime) + Q(t^\prime)] dt^\prime  \right\} , \tag{11} 
\end{align}
$$
...
knitr::kable(functions, col.names = c("", ""), escape = F) %>%
  ...
  column_spec(1, color = "white", 
              background = c(colpal2[2], colpal2[3], colpal2[4], colpal2[5], 
                             colpal2[6])) %>%
  add_header_above(c("Table 1.3: Defining the System Functions" = 2), 
                   align = "l", line = T)  %>%
  kable_paper()
```
...
#### <span class="sp7">6.</span> Time Coarse Graining
...
knitr::kable(parameters, col.names = c("", ""), escape = F) %>%
  ...
  add_header_above(c("Table 1.4: Defining the System Parameters" = 2), 
                   align = "l", line = T)  %>%
  kable_paper()
```
...
## Phase Plane Analysis, Steady-State Solutions, and Limit Cycles
...
knitr::kable(y, col.names = c("","","",""), escape = F, booktabs = T, linesep="") %>%
  ...
  add_header_above(c("Table 1.5: Defining the System Parameters" = 4), 
                   align = "l", line = T)  %>%
  kable_paper()
```
